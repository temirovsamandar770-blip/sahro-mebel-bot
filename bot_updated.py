...
DEFAULT_SETTINGS = {
    ...
    "products_footer": "📞 Buyurtma yoki savol uchun pastdagi aloqa tugmalaridan foydalaning.",
}
...
def contact_menu():
    ...
    for i in range(1, 5):
        ...
        if phone:
            b.button(text=f"📞 Menejer {i}", callback_data=f"contact_phone_{i}")
...
def order_contact_menu():
    ...
    for i in range(1, 5):
        phone = get_setting(f"phone_{i}")
        if phone:
            b.button(text=f"📞 {phone}", url=f"tel:{phone}")
    tg = get_setting("telegram_username")
    if tg:
        b.button(text="✈️ Telegram", url=f"https://t.me/{tg.lstrip('@')}")
    ig = get_setting("instagram")
...
async def category_chosen(call: CallbackQuery):
    ...
    footer = get_setting("products_footer")
    if footer:
        await call.message.answer(footer, reply_markup=order_contact_menu())
...
async def menu_aloqa(call: CallbackQuery):
    await call.message.edit_text(
        "👨‍💼 Menejerlarimiz bilan bog'laning:\nRaqamni bosing — qo'ng'iroq qiling.",
        reply_markup=contact_menu(),
    )


@user_router.callback_query(F.data.startswith("contact_phone_"))
async def contact_phone(call: CallbackQuery):
    i = call.data.split("_")[-1]
    phone = get_setting(f"phone_{i}")
    if not phone:
        await call.answer("Bu menejer raqami sozlanmagan.", show_alert=True)
        return
    await call.answer()
    await call.message.answer(f"📞 Menejer {i}: {phone}\n\nRaqamni nusxalab qo'ng'iroq qiling.")


@user_router.callback_query(F.data == "menu_zakaz")
async def menu_zakaz(call: CallbackQuery):
    await call.message.edit_text(
        "🛒 Zakaz berish uchun menejerlarimiz bilan bog'laning:",
        reply_markup=order_contact_menu(),
    )
...
PROMPTS = {
    "yotoqxona": "luxury modern bedroom furniture showroom, elegant wardrobe, bed and bedside tables, premium materials, warm indirect lighting, photorealistic architectural photography",
    "mehmonxona": "premium modern living room furniture showroom, elegant sofa, TV wall, coffee table and storage furniture, sophisticated interior, photorealistic architectural photography",
    "oshxona": "premium modern fitted kitchen furniture, practical cabinets, marble worktop, built-in appliances, stylish dining area, photorealistic architectural photography",
    "yumshoq": "premium upholstered sofa and armchair living room set, elegant modern interior, realistic fabric texture, warm cinematic lighting, photorealistic architectural photography",
    "bolalar": "beautiful modern children's bedroom furniture, practical wardrobe, bed, study desk, soft colors, safe cozy interior, photorealistic architectural photography",
}
...
async def ai_category(call: CallbackQuery, state: FSMContext):
    ...
    b = InlineKeyboardBuilder()
    b.button(text="🎨 Dizayn yaratish", callback_data="ai_generate")
    b.button(text="⬅️ Orqaga", callback_data="menu_ai")
    b.adjust(1)
    await call.message.edit_text(
        "🤖 AI dizayner\n\nTanlangan xona uchun professional mebel interyeri yaratamiz.",
        reply_markup=b.as_markup(),
    )


@ai_router.callback_query(F.data == "ai_generate")
async def ai_generate_callback(call: CallbackQuery, state: FSMContext):
    await call.answer()
    await ai_generate(call.message, state)
...
async def ai_generate(message: Message, state: FSMContext):
    ...
    await message.answer("🎨 Professional AI dizaynlar tayyorlanmoqda...")
    media = []
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as session:
            ...
            for i in range(4):
                ...
                prompt = f"{base_prompt}, variation {i + 1}, high-end furniture catalog, ultra detailed, clean composition, no people, no text, no watermark"
                url = "https://image.pollinations.ai/prompt/" + prompt.replace(" ", "%20") + f"?width=1024&height=683&seed={seed}&model=flux&nologo=true"
                tasks.append(fetch_image(session, url))
            results = await asyncio.gather(*tasks, return_exceptions=True)
        for idx, res in enumerate(results):
            if isinstance(res, Exception):
                continue
            media.append(InputMediaPhoto(media=BufferedInputFile(res, filename=f"ai_design_{idx + 1}.jpg"), caption="🤖 AI dizayn" if idx == 0 else None))
    except Exception:
        media = []
    if media:
        ...
        footer = get_setting("products_footer")
        if footer:
            await message.answer(footer, reply_markup=order_contact_menu())
    else:
        await message.answer("Kechirasiz, AI dizaynni yaratishda xatolik bo'ldi. Birozdan keyin qayta urinib ko'ring.")
    await state.clear()
...
async def menu_admin(call: CallbackQuery):
    ...
    b.button(text="📌 Mahsulotlar ostidagi yozuv", callback_data="adm_footer")
...
@admin_router.callback_query(F.data == "adm_footer")
async def adm_footer(call: CallbackQuery, state: FSMContext):
    current = get_setting("products_footer")
    await state.update_data(which="products_footer")
    await state.set_state(EditValue.value)
    await call.message.edit_text("📌 Mahsulot rasmlari tugagandan keyin chiqadigan yozuvni yuboring.\n\nHozirgi yozuv:\n" + current + "\n\nO'chirish uchun: -")
...
async def adm_value_save(message: Message, state: FSMContext):
    ...
    if data["which"] == "products_footer" and value == "-":
        value = ""
    set_setting(data["which"], value)
